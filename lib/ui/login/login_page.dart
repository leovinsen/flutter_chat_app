    ...
    if (form.validate()) {
      ...
      try {
        switch(_formType){
          case FormType.login:
            //String
            ResultMessage rm = await Auth.instance.signIn(_email, _password);

            if(rm.code == ResultCode.success) widget.onSignIn();
            else {
              createDialog(context, 'Error', rm.message);
            }
            break;
          case FormType.register:
            Auth.instance.createUser(_email, _password).then((uniqueId){
